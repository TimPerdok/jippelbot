...
export default class GameReleasesEmbedUpdater extends ScheduledActionWrapper {
   
    gameDataHandler: JSONDataHandler<Game[]>;
    serverdataHandler: JSONDataHandler<ServerConfig>;

    constructor(private serverId: string) {
        super(Interval.DAILY)
    ...
    async createMessage() {
        const serverdata = await this.serverdataHandler.getOfServer(this.serverId);
    ...
    async getContent(): Promise<string | MessageEditOptions | MessagePayload> {
        const games = await this.gameDataHandler.getOfServer(this.serverId);
    ...
    async run() {
        let subscribedGames = await this.gameDataHandler.getOfServer(this.serverId);
